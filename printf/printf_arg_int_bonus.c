...
static int	ft_arg_int_wchar(int n, int flag, int width)
{
	char	*output;
	int		ret;
	int		len;
...
static int	ft_arg_int_c(int n, int flag, int width, char *len)
{
	char	*c;
	int		ret;

	if (len && ft_strncmp(len, "l", 2) == 0)
	{
		if ((ret = ft_arg_int_wchar(n, flag, width)) == -1)
			return (-1);
		return (ret);
	}
	if (width == 0)
...
int			ft_arg_int(long long int n, const char id, t_flags p)
{
	int ret;

	if (id == 'c')
	{
		if ((ret = ft_arg_int_c(n, p.flag, p.width, p.len)) == -1)
			return (-1);
		return (ret);
	}
	else if (n < 0)
